...
import { neonVine } from '../src/biomes/neon-vine';
...
function makeGarden(events: GrowthEvent[]): Garden {
  return {
    ...
    config: { theme: 'neon-vine', width: 800, height: 600 },
  };
}

describe('neonVine.render', () => {
  it('renders a well-formed svg document', () => {
    const svg = neonVine.render(makeGarden([]));
    expect(svg).toMatch(/^<svg xmlns="http:\/\/www\.w3\.org\/2000\/svg"/);
    expect(svg).toContain('</svg>');
  });

  it('does not crash on an empty event list', () => {
    expect(() => neonVine.render(makeGarden([]))).not.toThrow();
  });
  ...
  it('does not crash on a single growth event', () => {
    ...
    expect(() => neonVine.render(makeGarden(events))).not.toThrow();
  });
  ...
  it('renders every event kind without throwing', () => {
    ...
    const svg = neonVine.render(makeGarden(events));
    expect(svg).toContain('</svg>');
  });
  ...
  it('produces the same output for the same seed (same events)', () => {
    ...
    const svgA = neonVine.render(makeGarden(events));
    const svgB = neonVine.render(makeGarden(events));
  ...
  it('produces different output for a different seed', () => {
    ...
    const svgA = neonVine.render(makeGarden(eventsA));
    const svgB = neonVine.render(makeGarden(eventsB));
    expect(svgA).not.toBe(svgB);
  });

  it('renders a visibly different aesthetic from bonsai for the identical Garden', async () => {
    const { bonsai } = await import('../src/biomes/bonsai');
    const events: GrowthEvent[] = [
      { kind: 'growth', intensity: 0.5, age: 0.5, origin: 0.3, timestamp: 1, meta: { sha: 'a'.repeat(40), author: 'solo' } },
    ];
    const garden = makeGarden(events);
    const vineSvg = neonVine.render(garden);
    const bonsaiSvg = bonsai.render({ ...garden, config: { ...garden.config, theme: 'bonsai' } });
    expect(vineSvg).not.toBe(bonsaiSvg);
  });
  ...
  it('respects width/height overrides from Garden config', () => {
    ...
    const svg = neonVine.render(garden);
